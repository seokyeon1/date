...
export default function CourseMap({ stops }) {
  ...
  if (status === "no-key") {
    return (
      <div className="rounded-xl border border-dashed border-slate-300 bg-slate-50 p-4 text-xs text-slate-400 mb-4">
  ...
  return (
    <div className="mb-4">
      <div
        ref={containerRef}
        className="w-full h-64 rounded-xl border border-slate-200 bg-slate-100"
      />
      {status === "error" && (
