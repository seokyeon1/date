...
function LoadingState() {
  return (
    <div className="flex flex-col items-center justify-center py-20 text-center">
      ...
      <p className="font-medium text-slate-700">실제 장소를 찾고 있어요</p>
...
function ErrorState({ message, onRetry }) {
  return (
    <div className="flex flex-col items-center justify-center py-20 text-center">
      ...
      <p className="font-medium text-slate-700">코스를 만드는 중 문제가 발생했어요</p>
      <p className="text-sm text-slate-400 mt-1 max-w-sm">{message}</p>
      <button
        onClick={onRetry}
        className="mt-5 px-5 py-2 rounded-lg bg-rose-500 text-white text-sm font-medium"
      >
        다시 시도
      </button>
      <p className="text-xs text-slate-300 mt-3">문제가 계속되면 잠시 후 다시 시도해주세요.</p>
    </div>
  );
}
...
function EmptyState({ message, onExpandRadius, onRetry }) {
  return (
    <div className="flex flex-col items-center justify-center py-20 text-center">
      <p className="text-3xl mb-3">🔍</p>
      <p className="font-medium text-slate-700">후보 장소가 부족해요</p>
      <p className="text-sm text-slate-400 mt-1 max-w-sm">{message}</p>
      <div className="flex gap-2 mt-5">
        ...
        <button
          onClick={onRetry}
          className="px-5 py-2 rounded-lg border border-slate-300 text-slate-600 text-sm font-medium"
        >
          입력 다시 조정
        </button>
      </div>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <button onClick={onBackToForm} className="text-sm text-slate-400 hover:text-slate-600">
          ← 새로 만들기
        </button>
