...
export default function InputForm({ onSubmit, submitting }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8">
      {/* 1. 데이트 시간대 */}
      <section>
        <h3 className="text-sm font-semibold text-slate-700 dark:text-slate-200 mb-2">1. 데이트 시간대</h3>
        ...
        <div className="flex items-center gap-3">
          <input
            ...
            className="border border-rose-200 dark:border-rose-900/40 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-rose-300 focus:border-rose-300 dark:focus:ring-rose-800 dark:focus:border-rose-800 transition-colors"
          />
      ...
      {/* 2. 지역 */}
      <section>
        <h3 className="text-sm font-semibold text-slate-700 dark:text-slate-200 mb-2">2. 지역</h3>

        <div className="flex border-b border-rose-100 dark:border-rose-900/30 mb-4" role="tablist">
          <button
            type="button"
            role="tab"
            ...
            className={`flex-1 text-sm font-medium py-2.5 border-b-2 -mb-px transition-colors ${
              regionMode === "district"
                ...
                : "border-transparent text-rose-700 dark:text-rose-400 hover:text-rose-800 dark:hover:text-rose-300"
            }`}
          >
            지역으로 찾기
          </button>
          ...
          <>
            ...
            <div className="flex flex-wrap gap-3 mb-3">
              <select
                ...
                className="border border-rose-200 dark:border-rose-900/40 rounded-lg px-3 py-2 text-sm bg-white dark:bg-zinc-900 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-rose-300 focus:border-rose-300 dark:focus:ring-rose-800 dark:focus:border-rose-800 transition-colors"
              >
              ...
              <select
                ...
                disabled={!sido}
                className="border border-rose-200 dark:border-rose-900/40 rounded-lg px-3 py-2 text-sm bg-white dark:bg-zinc-900 dark:text-slate-200 disabled:opacity-50 focus:outline-none focus:ring-2 focus:ring-rose-300 focus:border-rose-300 dark:focus:ring-rose-800 dark:focus:border-rose-800 transition-colors"
              >
            ...
            {!sido && (
              <p className="text-xs text-rose-700 dark:text-rose-400">지역을 선택해야 코스를 생성할 수 있어요.</p>
            )}
          </>
        ) : (
          <>
            <select
              ...
              className="border border-rose-200 dark:border-rose-900/40 rounded-lg px-3 py-2 text-sm bg-white dark:bg-zinc-900 dark:text-slate-200 mb-3 focus:outline-none focus:ring-2 focus:ring-rose-300 focus:border-rose-300 dark:focus:ring-rose-800 dark:focus:border-rose-800 transition-colors"
            >
              <option value="">시/도 선택</option>
              {SIDO_LIST.map((s) => (
                <option key={s} value={s}>
      ...
      {/* 3. 교통수단 */}
      <section>
        <h3 className="text-sm font-semibold text-slate-700 dark:text-slate-200 mb-2">3. 교통수단 (복수 선택 가능)</h3>
        ...
        {showCustomTransport && (
          <div className="flex gap-2 mt-2">
            <input
              ...
              placeholder="예: 오토바이"
              className="border border-rose-200 dark:border-rose-900/40 rounded-lg px-3 py-1.5 text-sm flex-1 max-w-xs focus:outline-none focus:ring-2 focus:ring-rose-300 focus:border-rose-300 dark:focus:ring-rose-800 dark:focus:border-rose-800 transition-colors"
            />
            <button
              type="button"
              onClick={addCustomTransport}
              className="text-sm px-3 py-1.5 rounded-lg bg-rose-600 text-white hover:bg-rose-700 transition-colors cursor-pointer"
            >
              추가
            </button>
          </div>
      ...
      {/* 4. 이동 반경 */}
      <section>
        <h3 className="text-sm font-semibold text-slate-700 dark:text-slate-200 mb-2">4. 이동 반경</h3>
        ...
        />
        <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">선택 반경: {radiusKm}km</p>
      </section>
      ...
      {/* 6. 요청사항 */}
      <section>
        <h3 className="text-sm font-semibold text-slate-700 dark:text-slate-200 mb-2">6. 요청사항 (선택)</h3>
        <textarea
          value={freeRequest}
          onChange={(e) => setFreeRequest(e.target.value)}
          rows={3}
          placeholder={FREE_REQUEST_PLACEHOLDERS.join("\n")}
          className="w-full border border-rose-200 dark:border-rose-900/40 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-rose-300 focus:border-rose-300 dark:focus:ring-rose-800 dark:focus:border-rose-800 transition-colors"
        />
      </section>
      ...
      <button
        ...
        className="w-full py-3 rounded-xl bg-rose-500 text-white font-semibold shadow-md shadow-rose-200 hover:bg-rose-600 disabled:bg-slate-300 dark:disabled:bg-zinc-700 disabled:shadow-none disabled:cursor-not-allowed cursor-pointer transition-colors"
      >
